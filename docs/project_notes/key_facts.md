# Key Facts

## Site
- GitHub Pages: https://mparrett.github.io/genealogy
- Build: `just build-reports` (converts markdown to HTML via mistune)

## Structure
- Reports: `research/reports/*.md` → `research/reports/html/`
- Ancestry PDFs: `assets/ancestry/`
- Research notes: `research/`

## Matt's Direct Lineage (Mowrey Line)

```
Johann Peter Maurer (1709–1781) + Anna Sophia Germann — immigrated 1751 on ship Edinburgh
    └── Peter W. Mowrey (1760–1840) + Gibson(?) — Revolutionary War veteran, Battle of Guilford Court House
        └── Lewis R. Mowery (c.1801–1850) + Elizabeth Lisbee
            └── Moses Mansfield Mowery (1822–1904) + Margaret Witt
                └── Samuel S. Mowrey (1867–1904) + Effie Mae Coker (1878–1958)
                    └── Raymond Robert Mowrey (1899–1973) + Anne Barbara Gleeson (1903–1986)
                        └── Patricia Anne Mowrey (1929–2009) ← grandmother
                            └── Michele Birch/Parrett ← mother
                                └── Matt
```

**Notes:**
- Surname evolved: Maurer → Mowrey/Mowery as family moved south
- Johann Peter: German immigrant from Baumholder, Rheinland; probate Lancaster, PA 1781
- Peter W.: Pension S1568, DAR Ancestor #A131224; will in Knox Estate Book 7, p.17
- Raymond + Anne Barbara Gleeson marriage connects Mowrey and Gleeson lines

## Matt's Direct Lineage (Parrett Line)

```
Amos Parret (1820–1863) — died Civil War, Helena, AR
    └── John Anderson Parrett (1859–1938) + Mary Ellen Fisher — orphaned at 4
        └── Adam Amos Parrett (1885–1959) + Mary Ann Beeks — Cleveland tradesman
            └── Charles Edward Parrett (1908–1983)
                └── James Parrett Sr. ← grandfather
                    └── [parent]
                        └── Matt
```

**Notes:**
- John Anderson orphaned when father Amos died in Civil War (1863)
- Adam moved Indiana → Cleveland for factory work; lost two sons in 1951 drowning
- Draft bios exist for John Anderson and Adam Amos

## Family Lines
- **Mowery/Mowrey**: Tennessee → Texas → Arizona
- **Kuthe**: Germany (Vöhl, Hesse) → Missouri → Oregon → Arizona
- **Gleeson/Fitzgerald**: Ireland (Limerick) → Wisconsin → Arizona
- **Higgins/Knight**: Ireland (Dublin/Sligo) → Newark → San Francisco

## External Profiles
- **WikiTree**: https://www.wikitree.com/wiki/Parrett-439 (Matt's profile)
- **Ancestry Tree**: https://www.ancestry.com/family-tree/tree/170975160/family?cfpid=162218649017 (public tree)

## Image Styles
- **Bio illustrations**: Watercolor style, warm sepia tones, narrative moments
- **Location panels**: Hand-watercolored engraving style (linework + subtle color washes)
  - Reference: https://mirka-h.blogspot.com/2015/02/imprinting-american-west-engravings.html
  - Techniques: parallel lines, cross-hatching, stippling; watercolor applied to complement not overwhelm linework
- **Future**: May become a print book — keep high-res originals, design for dual web/print use

## Key Find a Grave Memorials
- **James Joseph Gleeson**: https://www.findagrave.com/memorial/39556693/james-joseph-gleeson
- **Maria Ann Gleeson**: Sacred Heart Cemetery, Mondovi, WI (memorial TBD)
